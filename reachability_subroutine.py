## % Apurva Badithela
## % 5/6/2020
## Optimal strategy synthesis for reachability goals.
...
from test_strategy_synthesis_3 import plot_grid_world
from test_strategy_synthesis_3 import base_plot

# Load data:
GEdges = pickle.load(open("GEdges.dat", "rb"))
...
def compute_winning_set(S, G):
    ...
    while preS != W0:
        W.append(W0)
        for s in preS:
            successor = min_successor(s, V, G) # returns successor with the least weight
    ...
    for s in W0:
        ...
        for ii in range(len(pre_s)):
            if (bool_pre_s[ii] and (pre_s[ii] not in pre_agent)):
                Vnew[pre_s[ii]] = max([V[succ] for succ in pre_s_succ[ii]])
...
def test_strategy(prp):
    ...
    min_win_index = V_win.index(min(V_win))
